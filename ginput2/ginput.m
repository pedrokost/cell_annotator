...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    while how_many ~= 0
        ...
        if(ptr_fig == fig)
            ...
            if(char == 13) || keydown == -1; % & how_many ~= 0)
...
try
    ...
    % for a second wait for button press, then check fig userdata
    % if no fig user data redo the waitforbuttonpress

    % keydown = waitforbuttonpress

    while 1
        keydown = waitforbuttonpressfor(2);
        if keydown == -1
